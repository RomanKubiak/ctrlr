...
struct LookAndFeelParamWrapper
{
	...
	LookAndFeelParamWrapper (Graphics &g_, const Rectangle<int> &areaIntConst_, bool isSeparator_, bool isActive_, bool isHighlighted_, bool isTicked_, bool hasSubMenu_, const String &text_, const String &shortcutKeyText_, const Drawable* drawableIcon_, const Colour *textColourPtr_) : g(&g_), areaIntConst(&areaIntConst_), isSeparator(isSeparator_), isActive(isActive_), isHighlighted(isHighlighted_), isTicked(isTicked_), hasSubMenu(hasSubMenu_), text(&text_), shortcutKeyText(&shortcutKeyText_), drawableIcon(drawableIcon_), textColourPtr(textColourPtr_) {}
