...
{
	if (owner.getOwner().getCtrlrManagerOwner().getEditor())
	{
		CtrlrMenuBarLookAndFeel *lf = owner.getOwner().getCtrlrManagerOwner().getEditor()->getMenuBarLookAndFeel();
...
void CtrlrPanelMIDIToolbar::buttonClicked (Button *button, const int itemId, const int buttonId)
{
	...
	switch (itemId)
	{
		...
		case OptionsGroup:
			...
			{
				case ButtonMisc1:
                    owner.getOwner().getCtrlrManagerOwner().getCommandManager().invokeDirectly (CtrlrEditor::showMidiLibrary, true);
                    break;
...
{
	PopupMenu m;
	StringArray dev = owner.getOwner().getCtrlrManagerOwner().getCtrlrMidiDeviceManager().getManagedDevices (outputDevice);
...
{
	PopupMenu m;
	StringArray dev = owner.getOwner().getCtrlrManagerOwner().getCtrlrMidiDeviceManager().getManagedDevices (inputDevice);
